...
export default function Login() {
  ...
  const { clinicid, specialtyid } = router.query;
  ...
  const fetchSpecialty = async () => {
    if (specialtyid) {
      ...
      if (specialtyError) {
        ...
        setMessage({ type: "error", text: "Failed to load specialty" });
      } else {
      ...
      const { data: medicationsData, error: medicationsError } =
        await supabase.from("medications").select("*").order("id");
    ...
    const fetchConditionsAndMedications = async () => {
      if (specialty?.id) {
        ...
        const { data: conditionsData, error: conditionsError } = await supabase
          .from("conditions")
          .select("*")
          .eq("id", specialty.condition_id[0]);
          // .eq("id", specialty.condition_id);
        ...
        .insert([
          {
            ...
            formid:
              formType === "initial" ? "INITIAL_FORM_ID" : "CONTINUING_FORM_ID",
          ...
          <form onSubmit={handleOnboarding} className="space-y-4">
            ...
            <div className="mb-4 relative z-20">
              <Listbox value={condition} onChange={onConditionChange}>
                <ListboxLabel className="block text-gray-700 mb-2">
                  Condition
                </ListboxLabel>
                  ...
                  <ListboxButton className="relative w-full bg-white border border-gray-300 rounded py-2 pl-3 pr-10 text-left cursor-default focus:outline-none focus:ring-2 focus:ring-sky-600 focus:border-sky-600">
                    <span className="block truncate">
                      {conditions.find((c) => c.id === condition)
                        ?.conditionName || "Select Condition"}
                    </span>
            ...
            <div className="mb-4 relative z-10">
              ...
              >
                ...
                <div className="relative">
                  <ListboxButton className="relative w-full bg-white border border-gray-300 rounded py-2 pl-3 pr-10 text-left cursor-default focus:outline-none focus:ring-2 focus:ring-sky-600 focus:border-sky-600">
                    <span className="block truncate">
                      {medications.find((m) => m.id === medication)?.name ||
                        "Select Medication"}
                    </span>
                    <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
                      <svg
            ...
            <div className="mb-4">
              <label className="block text-gray-700 mb-2">
                Is this your first prescription?
              </label>
              <div className="flex">
                <button
                  type="button"
